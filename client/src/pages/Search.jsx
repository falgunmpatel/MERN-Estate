  ...
  return (
    <div className="flex flex-col md:flex-row">
      ...
      <div className="flex-1">
        <h1 className="text-3xl p-3 border-b text-slate-700 font-semibold mt-5">
          Listing results:
        </h1>
        <div className="p-7 flex flex-wrap gap-4">
            {!loading && listings.length === 0 && <p className="text-slate-700 text-xl p-7">No listings found!</p>}

            {loading && <p className="text-center text-slate-700 text-xl">Loading...</p>}

            {
                !loading && listings && listings.map((listing) => <ListingItem key={listing._id} listing={listing} />)
            }

            {showMore && <button onClick={handleShowMore} className="text-green-700 rounded-lg p-7 hover:underline w-full text-center">Show More</button>}
        </div>
      </div>
    </div>
  );
